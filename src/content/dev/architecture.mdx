...
token 只在握手时校验一次，所以换发 token 或删除节点时 hub 会主动把那条连接踢掉。
...
注册接口另认一个 `X-Node-Token` 头：安装器重跑时带上机器已有的 token，它仍对应节点就原样返回，不看窗口和
key，所以窗口关了也能重跑；对应不上再按 key 注册。
...
PUT    /api/nodes/batch            POST /api/nodes/delete
