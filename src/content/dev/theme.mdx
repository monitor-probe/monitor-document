...
| `config` | 否 | 站长可调的设置，面板按它画表单。见下文[主题设置](#主题设置) |
...
| `label` | 否 | 表单上显示的名称，不写就显示 `key` |
| `help` | 否 | 表单上的一行说明 |
| `options` | `select` 必需 | `[{ "value": "...", "label": "..." }]`，`value` 是非空字符串，`label` 不写就显示 `value` |
...
`select` 缺选项、分组标题没有 `label` 都算。改完 `theme.json` 装到 hub 上打开设置看一眼，少了哪一项就是哪一项写错了。
...
// 和面板同一套检查。保存的值可能来自这个主题的旧版本，对不上当前的声明就当没存过
...
const fields = (manifest.config as Field[]).filter((f) => f.type !== "title")
...
```ts
// 站长登录时才有权限，未登录得到 401
await fetch(`/api/themes/${manifest.short}/config`, {
  method: "PUT",
