...
- **网卡地址**：agent 从本机网卡上取，每个协议族一个，公网地址优先于内网、ULA 和 CGNAT。IPv6
  不取隐私扩展生成的临时地址，也不取换前缀后已经过期的旧地址
- **出口**：hub 看到的连接来源，标着「出口」。只在节点这个协议族的网卡上只有内网地址时显示，也就是 NAT
  或代理的出口，它不在节点上

NAT 机器的公网 IPv4 只存在于服务商的 NAT 网关上，网卡上没有，hub 也只有在节点经 IPv4
连过来时才看得到。所以网卡上只有内网 IPv4 的节点，agent 连 hub 时先走 IPv4，5 秒连不上再换 IPv6。
...
- 网关做了分流或透明代理时，去 hub 的连接从代理出去，显示的出口就是代理的。节点没有公网 IPv6
  的话，国家码也只能按代理出口算
- 纯 IPv6 机器经 NAT64 或 WARP 访问 IPv4 时，那个 IPv4 出口是很多人共用的转换地址，不显示
