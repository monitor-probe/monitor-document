...
面板里一键加的反向代理可以直接用，上游填 `127.0.0.1:28080`。它生成的模板里没有
`X-Forwarded-Proto`，会话 cookie 因此拿不到 `Secure` 标志，在站点配置的 `location ^~ /` 块里补
一行：
...
改站点的配置文件，不要改完再从反向代理的表单里保存一次——那会重新生成这段配置，这一行会丢。
...
- 没有 `X-Forwarded-For`，登录限流按代理地址计数，所有人共用一个桶；节点的连接来源也记成代理地址，
  NAT 节点在面板上看不到出口地址，网卡上没有公网地址的节点也查不出国家码
...
面板里「添加节点」能点，就说明你是从 HTTPS 域名进的——从 IP 或明文入口进后台时它是灰的。hub 看的
是浏览器发来的 `Origin`，不看 `Host` 和 `X-Forwarded-Proto`，所以别让反代清掉或改写 `Origin`。

`X-Forwarded-Proto` 配没配，看登录之后的会话 cookie 有没有 `Secure` 标志：浏览器开发者工具的
